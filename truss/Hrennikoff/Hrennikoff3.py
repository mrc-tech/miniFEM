...
# MODELLO CONTINUO
# -------------------------------------------------------------------------------------------------
from miniFEM import tri2D as tri2D
...
for iy in range(Ny):
    for ix in range(Nx):
        ...
        j = (Nx+1)*(iy+0) + ix+1
        k = (Nx+1)*(iy+1) + ix+1
        elements.append({'i':i, 'j':j, 'k':k, 'E':200e6, 'ni':1/3, 't':0.1})
        i = (Nx+1)*(iy+0) + ix+0
        j = (Nx+1)*(iy+1) + ix+0
        k = (Nx+1)*(iy+1) + ix+1
        elements.append({'i':i, 'j':j, 'k':k, 'E':200e6, 'ni':1/3, 't':0.1})
constraints = [{'id':0, 'u':True, 'v':True}] # 'u':True vuol dire che lo spostamento lungo x è vincolato
for i,n in enumerate(nodes):
    if n['y'] == 0:
        constraints.append({'id':i, 'u':False, 'v':True})
forces = []
...
for i,_ in enumerate(nodes):
    nodes[i]['u'] = u[i*dof+0].item()
    nodes[i]['v'] = u[i*dof+1].item()
tri2D.plot_displacement(nodes, elements, scale=1000000)