...
L = 16 # lunghezza totale trave reticolare
N = 8 # numero di elementi in cui e' suddiviso il corrente principale (superiore) [divisibile per 2]
r = 1.0 # rapporto tra la lunghezza degli elementi dei correnti e quelli delle aste (r = H/Dx)
pd = 10 # peso distribuito sulla trave [kN/m]
E = 210e6 # modulo di Young delle aste
A = 0.1 # area delle aste

P = pd*L # peso totale applicato alla trave
Dx = L/N # distanza tra i nodi dei correnti
# H = r*Dx # altezza trave
H = 2


nodes = []
for i in range(N+1): nodes.append({'x':  i   *Dx, 'y':  0}) # nodi superiori
for i in range(N-1): nodes.append({'x': (i+1)*Dx, 'y': -H}) # nodi inferiori

elements = []
for i in range(N): elements.append({'i':i, 'j':i+1, 'E':E, 'A':A}) # corrente superiore
for i in range(N-2): elements.append({'i':i+N+1, 'j':i+N+2, 'E':E, 'A':A}) # corrente inferiore
for i in range(N-1): elements.append({'i':i+1, 'j':i+N+1, 'E':E, 'A':A}) # aste di parete (verticali)
for i in range(int(N/2)): elements.append({'i':i, 'j':i+N+1, 'E':E, 'A':A}) # diagonali (sinistra)
for i in range(int(N/2)): elements.append({'i':i+1+int(N/2), 'j':i+N+int(N/2), 'E':E, 'A':A}) # diagonali (destra)
...
forces = []
for i in range(N-1): forces.append({'id':i+1, 'f':[0,-P/(N-1)]})
...
print('abbassamento massimo:', nodes[N//2]['v'])
...
if 'u' in nodes[0]: # controlla se ci sono gli spostamenti da mostrare
    ...
    for i,n in enumerate(nodes):
        scale = 0.1 # scala visualizzazione
...
def plot_forces(nodes, forces):
    for f in forces:
        ...
        plt.text(x - Fx*scale/2, y - Fy*scale/2, f"({Fx:.1f},{Fy:.1f})", color='red')
