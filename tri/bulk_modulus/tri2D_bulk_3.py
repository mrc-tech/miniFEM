'''
    in questa terza versione cerco di aggiungere l'analisi modale
    PROBLEMI:
        - cambiando H e B ottengo cose strane (devo sempre giostrarmela con quella costante strana 0.5 ...).
        - il periodo è dipendente dallo spessore del modello (forse con il circolare 3D va meglio..?)
        - il periodo è dipendente dalla discretizzazione del modello
'''
...
t_ele = 1 # spessore elementi
gamma_l = 8.57 # peso specifico del fluido interno (benzene = 8.57 kN/m^3)
...
for i in range(5):
    print(f"Modo {i+1}: T = {periods[i]:.3f} s")
