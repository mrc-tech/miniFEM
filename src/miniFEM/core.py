...
dof = 2 # Gradi di libertà per nodo... PERO' IN QUESTO MODO NON POSSO DEFINIRE ELEMENTI 3D!!!!!!!!!


def init_system(num_nodes):
    ''' Inizializza la matrice di rigidezza sparsa K e il vettore delle forze f. '''
    size = num_nodes * dof
    K = lil_matrix((size, size), dtype=np.float64) # lil_matrix è la più veloce per inserire/modificare elementi uno ad uno (?)
...
def add_constraint(K, f, constraint):
    ...
    if constraint.get('u', False): # Vincolo orizzontale
        ...
        K[:, idx] = 0 # <--- CRITICO !!!!
        '''
            LIL sta per List of Lists, ed è una struttura dati salvata per righe (utile per aggiungere nuovi dati velocemente).
            Quando scrivi K[idx,:] = 0, Python cancella una riga in una frazione di millisecondo.
            Ma quando scrivi K[:,idx] = 0 (cancellare una colonna), costringi SciPy a fare un ciclo su tutte le righe della matrice per cercare ed eliminare quel singolo elemento. Su una matrice 10.000 x 10.000, azzerare le colonne per applicare i vincoli puo' metterci tanto.
        '''
        K[idx, idx] = 1.0
...
def add_constraint_penalty(K, f, constraint):
    ''' Applica i vincoli usando il Metodo della Penalità per alte prestazioni con matrici sparse. '''
